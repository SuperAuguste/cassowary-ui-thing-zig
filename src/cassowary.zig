    ...
    return struct {
        ...
        fn optimize(solver: *@This(), allocator: Allocator) !void {
            while (true) {
                ...
                const exit_row = solver.rows.get(exit_variable).?;

                std.log.info("simplex: enter {any}, exit {any}", .{ entry_variable, exit_variable });
        ...
        fn dualOptimize(solver: *@This(), allocator: Allocator) !void {
            while (true) {
                ...
                for (
                    exit_row.terms.keys(),
                    exit_row.terms.values(),
                ) |variable, coefficient| {
                    if (coefficient <= 0) continue;

                    const new_min =
                        (solver.objective_function.terms.get(variable) orelse continue) /
                        coefficient;
                ...
                try exit_row.addTerm(allocator, exit_variable, -1);
                exit_row.solveFor(entry_variable);
                _ = exit_row.terms.swapRemove(entry_variable);

                try solver.objective_function.replaceVariable(
                    allocator,
                    exit_row,
                    entry_variable,
                );
        ...
        pub fn suggestValue(
            ...
            if (solver.rows.getPtr(edit_variable.positive_error)) |error_variable_positive_row| {
                ...
                for (solver.rows.values()) |*dest_row| {
                    dest_row.constant +=
                        delta * (dest_row.terms.get(edit_variable.positive_error) orelse continue);
                }
            }
